...
/*   Updated: 2025/03/02 22:36:44 by bolcay           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (game->map[row])
	{
		...
		while (game->map[row][col])
		{
			if (game->map[row][col] == '0' || game->map[row][col] == 'P'
				...
				mlx_image_to_window(game->mlx, floor, (col * TS), (row * TS));
			col++;
		}
		row++;
	}
	...
	while (game->map[row])
	{
		...
		while (game->map[row][col])
		{
			if (game->map[row][col] == '1')
				mlx_image_to_window(game->mlx, wall, (col * TS), (row * TS));
			col++;
		}
		row++;
	}
...
void	map_collectable(t_game *game)
{
	...
	c = mlx_texture_to_image(game->mlx, collectable_image);
	if (!c || !collectable_image)
		return ;
	row = 0;
	while (game->map[row])
	{
		...
		while (game->map[row][col])
		{
			if (game->map[row][col] == 'C')
				mlx_image_to_window(game->mlx, c, (col * TS), (row * TS));
			col++;
		}
		row++;
	}
	...
	while (game->map[row])
	{
		...
		while (game->map[row][col])
		{
			if (game->map[row][col] == 'E')
				mlx_image_to_window(game->mlx, exit, (col * TS), (row * TS));
			col++;
		}
		row++;
	}
