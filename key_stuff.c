...
/*   Updated: 2025/02/28 20:37:14 by batuhan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	move_up(t_game *game, t_player *player)
{
	...
	if (game->map[player->y][player->x] == 'C')
	{
		...
		map_background(game, (player->y + 1), player->x);
		player_image(game, player->y, player->x);
		game->map[player->y][player->x] = '0';
	}
	else if (game->map[player->y][player->x] == '1')
	{
		ft_printf("bruh... you can't go there...\n");
		game->player->y++;
	}
	else if (game->map[player->y][player->x] == 'E' && game->collectable == game->collectable_count)
	{
		ft_printf("congrats!!\n");
		exit(1);
	}
	...
	else if (game->map[player->y][player->x] == '0')
	{
		map_background(game, (player->y + 1), player->x);
		player_image(game, player->y, player->x);
	}
...
int	move_right(t_game *game, t_player *player)
{
	...
	if (game->map[player->y][player->x] == 'C')
	{
		ft_printf("a motivational message for collecting something!!!\n");
		game->collectable_count++;
		map_background(game, player->y, (player->x - 1));
		player_image(game, player->y, player->x);
		game->map[player->y][player->x] = '0';
	}
	else if (game->map[player->y][player->x] == '1')
